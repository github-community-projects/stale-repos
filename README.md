...
| `INCLUDE_CUSTOM_PROPERTIES` | false    |            | Comma separated list of [repository custom properties](https://docs.github.com/en/organizations/managing-organization-settings/managing-custom-properties-for-repositories-in-your-organization) used to restrict the scan. When set, only repositories matching are considered; when unset, all repositories are scanned. Each entry is either `name` (matches when the property is set to any value) or `name=value` (matches that value; for multi-select properties, matches when `value` is one of them). Comparison is case-insensitive and all entries must match, e.g. `INCLUDE_CUSTOM_PROPERTIES = "owner=my-team,lifecycle=production"`. Requires `ORGANIZATION` to be set and a token with `read:org` |
    ...
    steps:
      ...
      - name: Run stale_repos tool
        ...
        env:
          ...
          INCLUDE_CUSTOM_PROPERTIES: "owner=my-team"
          INACTIVE_DAYS: 365
...
1. (Optional) Fill out the `.env` file with the [repository custom properties](https://docs.github.com/en/organizations/managing-organization-settings/managing-custom-properties-for-repositories-in-your-organization) _include_custom_properties_ that you want to restrict the stale repos scan to. This should be a comma separated list of `name` or `name=value` entries, and requires _organization_ to be set.
